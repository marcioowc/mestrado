 ...
 */
public class CloudSimSDA1 {
	/** The cloudlet list. */
	private static List<Cloudlet> cloudletList;
	/** The vmlist. */
	private static List<Vm> vmlist;
        
        static private int NCloudlet = 5;
        static private int NVms = 2;
        static private int NHost = 1;
	...
	public static void main(String[] args) {
		Log.printLine("Starting CloudSimSDA1...");
		...
		try {
			...
			Cloudlet[] cloudlet = new Cloudlet[NCloudlet];
                        for (id=0; id<NCloudlet; id++) {
                                cloudlet[id] = new Cloudlet(id, length, pesNumber, fileSize, 
			...
			Log.printLine("CloudSimSDA1 finished!");
		} catch (Exception e) {
			e.printStackTrace();
	...
	private static Datacenter createDatacenter(String name) {
		...
		hostList.add(
			new Host(
                ...
                Host[] myhost = new Host[NHost];
                for (hostId=1; hostId < NHost; hostId++) {
                    myhost[hostId] = new Host (hostId,
                                                new RamProvisionerSimple(ram),
                                                new BwProvisionerSimple(bw),
                                                storage,
                                                peList,
                ...
                hostList.add(myhost[hostId]); // This is our machine
                }
			...
			if (cloudlet.getCloudletStatus() == Cloudlet.SUCCESS) {
				...
				Log.printLine(indent + indent + cloudlet.getResourceId()
						...
						+ dft.format(cloudlet.getFinishTime()) + indent
                                                + dft.format(cloudlet.getNumberOfPes()));
			}
		}
	}
}